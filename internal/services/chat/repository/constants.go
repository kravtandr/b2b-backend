...
const (
	...
	createGetChat         = "SELECT * FROM Chats WHERE product_id = $1 AND creator_id = $2"
	createNewChat         = "INSERT INTO Chats (name, creator_id, product_id) VALUES ($1,$2,$3) RETURNING id "
	createWriteNewMsg     = "INSERT INTO Msgs (chat_id,sender_id, receiver_id, text, type) VALUES ($1,$2,$3,$4,$5) RETURNING id, created_at"
	createGetMsgsFromChat = "SELECT id, chat_id, sender_id, receiver_id, checked, text, type, created_at FROM Msgs WHERE chat_id = $1 AND sender_id = $2 OR receiver_id = $2"
